...
import "../interface/ILiquidatorVault.sol";
...
abstract contract AbstractVaultManager is IVaultManager, Initializable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    ILiquidatorVault public cashStrategy;
...
// *********** TODO ***********
    ...
    function setCashStrategy(address _cashStrategy) public onlyAdmin {
        ...
        cashStrategy = ILiquidatorVault(_cashStrategy);
    ...
    function deposit() external override onlyVault cashStrategySet {

        console.log('deposit at vault Manager');
        console.log('checking balance for assset (%s)', address(asset));
        uint256 vmAssetBalance = asset.balanceOf(address(this));
        console.log('vmAssetBalance', vmAssetBalance);
        if (vmAssetBalance == 0) {
        ...
        console.log('transfer to cashStrategy (%s) ', address(cashStrategy), vmAssetBalance);
    ...
    returns (uint256, bool) {
        ...
        if (currentBalance < _amount) {
            revert(string(
                abi.encodePacked(
                    "In VaultManager not enough for transfer _amount: ",
    ...
    function _collectRewards() internal {
        ...
        for (uint8 i; i < strategies.length; i++) {
            ...
            if (item.enabledReward) {
                ILiquidatorVault(item.strategy).collectRewards();
            }
        }
    }
