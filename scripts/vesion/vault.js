...
async function main() {
  ...
  async function purchase(_amount) {
    ...
    await usdc.connect(signer).approve(sionVault.address, ethers.utils.parseEther("999999999999999999999999.0"));
    await sion.connect(signer).approve(sionVault.address, ethers.utils.parseEther("999999999999999999999999.0"));
  ...
  await purchase(ethers.utils.parseEther("99.0"));
  //await setup();
  await mine(4000);
  ...
  const sionVaultFractionToInvestDenominator = await sionVault.vaultFractionToInvestDenominator();
  const sionVaultFractionToInvestNumerator = await sionVault.vaultFractionToInvestNumerator();
  const strategyAddress = await sionVault.strategy();
  const underlyingAddress = await sionVault.underlying();
  const _underlyingUnit = await sionVault.underlyingUnit();
